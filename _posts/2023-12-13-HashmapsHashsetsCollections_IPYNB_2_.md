...
Set - Storing a set of username password pairs, where there can't be duplicate usernames
Map - Same as set
Queue - Browser back button
Deque - Browser back and forward buttons
...
public class ShoePrices {
    public static void main(String[] args) {
        ...
        HashMap<String, Object> hashMap = new HashMap<>(); // Change to Object
        ...
        // Retrieve the value, which is an Object
        Object value = hashMap.get("Nike");

        // Check if the value is an ArrayList
        if (value instanceof ArrayList) {
            // If it is, cast it to ArrayList
            ArrayList<Double> nikePricesFromMap = (ArrayList<Double>) value;

            // Print the prices
            System.out.println("Nike shoe prices: " + nikePricesFromMap);
        } else {
            // If it's not an ArrayList, it's a single Double value
            System.out.println("A Nike shoe would cost " + value);
        }
    }
}
...
findByEmail - Searches the repository for a person with the given email
findAllByOrderByNameAsc - Finds a name by ascending order
findByNameContainingIgnoreCaseOrEmailContainingIgnoreCase - Searches the repository for a name or email, case insensitive.
findByEmailAndPassword - Searches the repository for a person with the given email and password
findByLikeTermNative - Find users who have a term in common
...
A person can have a different role id, and multiple people can have the same role id. This means that the repository won't complain about having a duplicated role id, and that the role id can be used to find all the people with that role id. This is relevant to collections because it allows us to store multiple people in a single role id.
...
`nullable = false` This means that the field cannot be empty, and must have a value. This is relevant to the leftmost "id" column because it means that the id cannot be empty, and must have a value.
...
# HACKS
Hacks link: https://github.com/dolphinalt/APCSA-Pages/issues/6
