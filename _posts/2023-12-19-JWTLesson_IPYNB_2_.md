...
1. Header
    - The header typically consists of two parts: the type of the token, which is JWT, and the encryption that is used, such as HMAC SHA256 or RSA SHA256. It is Base64Url encoded to form the first part of the JWT
2. Payload
...
- header - defines type and algorithms used in the JWT
- payload - contains info about the user, issue date and expiration date.
- signature - a signing of the key used for encryption
- algorithm - the algorithm used to encrypt the JWT
- secret key - the server-side key used to sign the JWT
